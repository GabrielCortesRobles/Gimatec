<div class="main-menu menu-fixed menu-dark menu-accordion menu-shadow" data-scroll-to-active="true">
    <div class="main-menu-content">
        <ul class="navigation navigation-main" id="main-menu-navigation" data-menu="menu-navigation">
            <li class="nav-item">
                <a href=""><i class="icon-user-follow"></i>
                    <span class="menu-title"  data-i18n="nav.support_documentation.main">Nuevo Usuario</span>
                </a>
            </li>

            <li class="nav-item">
                <a href=""><i class="icon-settings"></i>
                    <span class="menu-title" data-i18n="nav.support_documentation.main">Perfil</span>
                </a>
            </li>
<!----------------------------------------Prospecto--------------------------------------------------------------->
            <li class="nav-item has-sub"><a href=""><i class="icon-eye"></i><span class="menu-title">Prospectos</span></a>
                <ul class="menu-content">
                    <li>
                        <a class="menu-item" href="{{route('nuevoprospecto')}}" data-i18n="nav.dash.ecommerce">
                            <i class="icon-plus">

                            </i>
                            Nuevo
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="{{route('reporteprospecto')}}" data-i18n="nav.dash.crypto">
                            <i class="icon-notebook">

                            </i>
                            Consulta
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="dashboard-sales.html" data-i18n="nav.dash.sales">
                            <i class="icon-note">

                            </i>
                            Editar
                        </a>
                    </li>
                </ul>
            </li>
            <!----------------------------------------Categoria--------------------------------------------------------------->
            <li class="nav-item has-sub"><a href=""><i class="icon-eye"></i><span class="menu-title">Categoria</span></a>
                <ul class="menu-content">
                    <li>
                        <a class="menu-item" href="{{route('nuevacategoria')}}" data-i18n="nav.dash.ecommerce">
                            <i class="icon-plus">

                            </i>
                            Nuevo
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="{{route('reportecategoria')}}" data-i18n="nav.dash.crypto">
                            <i class="icon-notebook">

                            </i>
                            Consulta
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="dashboard-sales.html" data-i18n="nav.dash.sales">
                            <i class="icon-note">

                            </i>
                            Editar
                        </a>
                    </li>
                </ul>
            </li>
<!----------------------------------------Maquinas--------------------------------------------------------------->
            <li class="nav-item has-sub"><a href=""><i class="icon-eye"></i><span class="menu-title">Maquina</span></a>
                <ul class="menu-content">
                    <li>
                        <a class="menu-item" href="{{route('nuevamaquina')}}" data-i18n="nav.dash.ecommerce">
                            <i class="icon-plus">

                            </i>
                            Nuevo
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="{{route('reportemaquina')}}" data-i18n="nav.dash.crypto">
                            <i class="icon-notebook">

                            </i>
                            Consulta
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="dashboard-sales.html" data-i18n="nav.dash.sales">
                            <i class="icon-note">

                            </i>
                            Editar
                        </a>
                    </li>
                </ul>
            </li>
        <!--------------------------------------Servicios--------------------------------------------------------->
            <li class="nav-item has-sub"><a href=""><i class="icon-eye"></i><span class="menu-title">Servicio</span></a>
                <ul class="menu-content">
                    <li>
                        <a class="menu-item" href="{{route('nuevoservicio')}}" data-i18n="nav.dash.ecommerce">
                            <i class="icon-plus">

                            </i>
                            Nuevo
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="{{route('reporteservicio')}}" data-i18n="nav.dash.crypto">
                            <i class="icon-notebook">

                            </i>
                            Consulta
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="dashboard-sales.html" data-i18n="nav.dash.sales">
                            <i class="icon-note">

                            </i>
                            Editar
                        </a>
                    </li>
                </ul>
            </li>

<!-----------------------------------------Empleado--------------------------------------->
            <li class="nav-item has-sub"><a href=""><i class="icon-eye"></i><span class="menu-title">Empleado</span></a>
                <ul class="menu-content">
                    <li>
                        <a class="menu-item" href="{{route('nuevo_empleado')}}" data-i18n="nav.dash.ecommerce">
                            <i class="icon-plus">

                            </i>
                            Nuevo
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="{{route('reporteempleado')}}" data-i18n="nav.dash.crypto">
                            <i class="icon-notebook">

                            </i>
                            Consulta
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="dashboard-sales.html" data-i18n="nav.dash.sales">
                            <i class="icon-note">

                            </i>
                            Editar
                        </a>
                    </li>
                </ul>
            </li>



            <li class="nav-item has-sub"><a href=""><i class="icon-eye"></i><span class="menu-title">Ventas</span></a>
                <ul class="menu-content">
                    <li>
                        <a class="menu-item" href="{{route('nuevamaquina')}}" data-i18n="nav.dash.ecommerce">
                            <i class="icon-plus">

                            </i>
                            Nuevo
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="" data-i18n="nav.dash.crypto">
                            <i class="icon-notebook">

                            </i>
                            Consulta
                        </a>
                    </li>
                    <li>
                        <a class="menu-item" href="dashboard-sales.html" data-i18n="nav.dash.sales">
                            <i class="icon-note">

                            </i>
                            Editar
                        </a>
                    </li>
                </ul>
            </li>



            <li class="nav-item">
            <a href=""><i class="icon-logout"></i>
            <span class="menu-title" data-i18n="nav.support_documentation.main">Cerrar Session</span>
            </a>
            </li>
        </ul>
    </div>
</div>